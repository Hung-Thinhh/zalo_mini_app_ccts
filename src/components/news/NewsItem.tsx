...
const Wrapper = styled.div`
  ${tw`flex flex-row cursor-pointer transition-transform duration-150`};
  position: relative;
...
const NewsItem: FC<NewsItemProps> = ({ data }) => {
  const { thumb, createdAt, title, link, thumb1 } = data;
  console.log('thumbbbb',data);
  

  const openNewsItem = useCallback(() => {
    if (!link) return;
    try {
      if (window.zalo && typeof openWebView === "function") {
        openWebView(link);
      } else {
